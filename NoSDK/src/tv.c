...
static void tv_line_int(void);


static void tv_sync_off_int(void)
{
    // Отключить синхру
    gpio_off(SYNC_PIN);
...
static void i2s_start_int(void)
{
    // Запускаем I2S и синхронизируем прерывание строки с началом передачи по I2S
    i2s_start();
    timer0_user_cb=tv_line_int;
    timer0_write(timer0_read()+64*160+480);	// второе число подбирается по логическому анализатору, чтобы трансфер I2S и таймер запускались одновременно
    gpio_on(SYNC_PIN);
}
